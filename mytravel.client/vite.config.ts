...
// Check if HTTPS mode is enabled (set VITE_HTTPS=true to enable HTTPS, default is HTTP-only)
const httpOnly = env.VITE_HTTPS !== 'true';

// SSL Certificate Setup for ASP.NET integration (only if not in HTTP-only mode)
